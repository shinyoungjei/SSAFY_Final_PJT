...
const qnaStore = {
  ...
  mutations: {
    ...
    CREATE_CHAT(state, payload) {
      state.qnaChat.push(payload);
    },
    DELETE_TODO(state, todoItem) {
      const index = state.qnaChat.indexOf(todoItem);
      state.qnaChat.splice(index, 1);
    },
    UPDATE_TODO_STATUS(state, content) {
      // console.log(state);
      // console.log(todoItem);
      state.qnaChat = state.qnaChat.map(todo => {
        if (todo === contentItem) {
          return {
