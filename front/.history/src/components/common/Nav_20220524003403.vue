<template>
  <b-navbar toggleable="lg" class="nav" type="light" variant="alert-primary">
    <b-navbar-brand href="#">
      <router-link to="/">
        <img
    ...
    <b-navbar-nav>
      ...
      <b-row v-if="loginUserInfo.id">
        ...
        <b-nav-item :to="{ name: `testmypage` }" class="mr-4">
          마이페이지
        </b-nav-item>
      </b-row>
      <b-row v-else>
        <b-nav-item :to="{ name: 'regist' }">회원가입</b-nav-item>
        <b-nav-item :to="{ name: 'login' }" class="mr-4">로그인</b-nav-item>
      </b-row>
    </b-navbar-nav>
  </b-navbar>
</template>
