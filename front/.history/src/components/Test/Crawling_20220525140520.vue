      ...
      <b-col>
        <b-row>
          ...
          <b-col class="text-right font-weight-bold">
            <a href="/notices" class="text-dark">더보기</a>
          </b-col>
        </b-row>
        <hr />
        <b-row>
          <b-col class="text-left">
            <ul style="list-style: none">
              <li v-for="(noticelist, i) in notices" v-bind:key="i">
                <div v-if="i <= 5">
                  <router-link
                    :to="`/notices/` + noticelist.noticeNo"
                    class="text-dark"
                    style="font-size: 15px"
                    >{{ noticelist.subject }}</router-link
                  >
                </div>
              </li>
            </ul>
...
const noticeStrore = "noticeStore";
export default {
  components: {
    Youth,
  },
  ...
  computed: {
    ...mapState(qnaStore, noticeStrore, ["questions", "notices"]),
  },
};
</script>
